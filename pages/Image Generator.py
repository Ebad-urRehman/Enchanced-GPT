...
from utils import image_generator_utils as utils
import time
import os
import glob
...
model_selected = st.sidebar.selectbox("", model_options)
...
""")
        ...
        model = "dall-e-3"
        selection_image_size = ["1024x1024", "1024x1792", "1792x1024"]
        selection_size = st.sidebar.radio("Select Size", selection_image_size)

        # size selection menu
        if selection_size == "1024x1024":
            size = "1024x1024"
        elif selection_size == "1024x1792":
            size = "1024x1792"
        elif selection_size == "1792x1024":
            size = "1792x1024"

        # quality select
        select_quality = ["Standard", "HD"]
        selected_quality = st.sidebar.radio("Select Quality", select_quality)
        ...
        user_input = utils.text_area_img()
        ...
        if Generate_button:
            if user_input == "":
                ...
                image_bot_response = image_bot.get_image(user_input, size, model, quality)
                ...
                def save_image():
                    ...
                    utils.download_image(image_bot_response, save_path)
...
if model_selected == "Dall.E-2":
    ...
    images = None
    if selected_option == "Generate new images":
        selection_image_type = ["Generate an Image", "✏️ Edit an Image", "🔄 Make Variations of Image"]
        selection = st.sidebar.selectbox("Selection Menu", selection_image_type)


        # when Edit image option selected
        if selection == "Generate an Image":
            directory_name = f"files/images/Creation/{date}"
            if not os.path.exists(directory_name):
                os.makedirs(directory_name)
                i = 0
    ...
    """)
        ...
        if selection == "✏️ Edit an Image":
            ...
            if selection_size == "256x256":
                size = "256x256"
                if user_image_actual:
                    user_img_actual_formatted = utils.format_image(user_image_actual, 256)
                    st.image(user_img_actual_formatted)
                    utils.save_image_to_disk(user_img_actual_formatted, temp_file_actual)
                    encoded_img_actual = utils.img_to_bytes(temp_file_actual)
                if user_image_masked:
                    user_img_masked_formatted = utils.format_image(user_image_masked, 256)
                    st.image(user_img_masked_formatted)
                    utils.save_image_to_disk(user_img_masked_formatted, temp_file_masked)
                    encoded_img_masked = utils.img_to_bytes(temp_file_masked)
            elif selection_size == "512x512":
                size = "512x512"
                if user_image_actual:
                    user_img_actual_formatted = utils.format_image(user_image_actual, 512)
                    st.image(user_img_actual_formatted)
                    utils.save_image_to_disk(user_img_actual_formatted, temp_file_actual)
                    encoded_img_actual = utils.img_to_bytes(temp_file_actual)
                if user_image_masked:
                    user_img_masked_formatted = utils.format_image(user_image_masked, 512)
                    st.image(user_img_masked_formatted)
                    utils.save_image_to_disk(user_img_masked_formatted, temp_file_masked)
                    encoded_img_masked = utils.img_to_bytes(temp_file_masked)
            elif selection_size == "1024x1024":
                size = "1024x1024"
                if user_image_actual:
                    user_img_actual_formatted = utils.format_image(user_image_actual, 1024)
                    st.image(user_img_actual_formatted)
                    utils.save_image_to_disk(user_img_actual_formatted, temp_file_actual)
                    encoded_img_actual = utils.img_to_bytes(temp_file_actual)
                if user_image_masked:
                    user_img_masked_formatted = utils.format_image(user_image_masked, 1024)
                    st.image(user_img_masked_formatted)
                    utils.save_image_to_disk(user_img_masked_formatted, temp_file_masked)
                    encoded_img_masked = utils.img_to_bytes(temp_file_masked)
            ...
            if Generate_button:
                if user_input == "":
                    ...
                    for j, image_response_link in enumerate(image_bot_response):
                        ...
                        utils.save_image(directory_name, img_name, i, image_response_link)
                        i += 1
                    st.success("✨ Image Generated Successfully✨")

        # deleting temporary file made for encoding
            if os.path.exists(temp_dir):
                utils.delete_all_images(temp_dir)
        ...
        # Make variation of images
            ...
            if selection_size == "256x256":
                size = "256x256"
                if user_image:
                    user_img_formatted = utils.format_image(user_image, 256)
                    st.image(user_img_formatted)
                    utils.save_image_to_disk(user_img_formatted, temp_file)
                    encoded_img = utils.img_to_bytes(temp_file)
            elif selection_size == "512x512":
                size = "512x512"
                if user_image:
                    user_img_formatted = utils.format_image(user_image, 512)
                    st.image(user_img_formatted)
                    utils.save_image_to_disk(user_img_formatted, temp_file)
                    encoded_img = utils.img_to_bytes(temp_file)
            elif selection_size == "1024x1024":
                size = "1024x1024"
                if user_image:
                    user_img_formatted = utils.format_image(user_image, 1024)
                    st.image(user_img_formatted)
                    utils.save_image_to_disk(user_img_formatted, temp_file)
                    encoded_img = utils.img_to_bytes(temp_file)
